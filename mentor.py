...
# ✨ [중요] API 요청 횟수를 줄이기 위한 캐싱 함수 (10초 동안은 시트 데이터를 다시 부르지 않음)
...
def fetch_latest_data():
    try:
        ...
        # API 에러 발생 시 앱이 멈추지 않도록 기존 데이터 유지 (혹은 안내)
        if "available_slots" not in st.session_state:
            st.error("⚠️ 구글 시트 연결이 일시적으로 원활하지 않습니다. 잠시 후 새로고침(F5) 해주세요.")
...
def safe_save(ws_name, data_list):
    try:
        ...
        st.error("⚠️ 저장 중 오류가 발생했습니다. 잠시 후 다시 시도해 주세요.")
    ...
    if m_log2 != "선택해주세요":
        ...
        if minfo and st.text_input("비밀번호 입력", type="password", key="m_pw_t2") == str(minfo['pw']):
            ...
            if st.button("🗓️ 일정 등록하기", type="primary", use_container_width=True, key="sb_t2"):
                ...
                st.snow(); time.sleep(1.5); st.rerun()

# --- [이하 탭 3, 4 동일 로직 적용] ---
with tab3:
    st.subheader("📋 멘티 신청 현황 관리")
    m_sel3 = st.selectbox("성함 선택", mentor_names, key="m_sel_t3", on_change=reset_pw_t3)
    if m_sel3 != "선택해주세요":
